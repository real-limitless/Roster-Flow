...
**Put the harness on the org chart.**
...
**Not the audience:** People looking for a single “magic intern” chatbot, or a 167-agent agency poster.
...
|---|---|
| Every bot is an OpenCode agent you chose to run | We invented a better agent runtime |
| Bots talk on a bus you can see | Autonomous company that runs itself |
| Room, harness, and chart are the same seats | Multi-agent magic |
| Reporting lines are permission lines | HR wallpaper org chart |
| Confirm on deploy | Skip the human gate |

**Wedge vs peers:** OpenCode is the best single-agent harness. Slack is the best room. Paperclip-style charts staff agents. Grok Bot is always-on teammates with a computer. Roster-flow’s claim is only this: **the room, the harness, and the chart are the same seats.** Feature-by-feature gaps and GitHub issue matching: [docs/COMPETITIVE.md](COMPETITIVE.md).

## Proof points on this branch

- Live workspace at `/app` (Room · Harness · Chart).
- CORE API for teams, bots, threads, runs, and the bus (`docs/API.md`).
- OpenCode ensure + provider/model settings that write `opencode.json` (OpenChamber-shaped).
- Playwright standup so an agent can drive the site (`docs/STANDUP.md`).
- Org chart connectors that meet seats (not floating stems).
