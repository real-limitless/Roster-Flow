...
Tools: `roster_list_seats`, `roster_send_message`, `roster_handoff`, `roster_report`, `roster_ask_human`. `roster_send_message` and `roster_report` accept optional Roster Block Kit `blocks` JSON. `roster_report` also accepts optional `prUrl` and `branch` for GitHub cards in `#ship`.
